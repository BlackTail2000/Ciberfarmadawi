<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.List, model.Tipo, dao.TipoDAO" %>
<%
...
<body class="p-5 bg-primary-subtle">
  <div class="p-5 border border-5 border-info rounded bg-white">
    ...
    <hr>
    <button class="btn btn-outline-success my-4" data-bs-toggle="modal" data-bs-target="#modal-create"><i class="bi bi-database-add"></i> Registrar Tipo</button>
    <div class="modal fade" id="modal-create" tabindex="-1" aria-labelledby="modal-titleC" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="modal-titleC">Registrar Nuevo Tipo</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <form action="TipoServlet">
    ...
    <div class="modal fade" id="modal-update<%=count%>" tabindex="-1" aria-labelledby="modal-titleU<%=count%>" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="modal-titleU<%=count%>">Actualizar Tipo Nro. <%=item.getIdtipo() %></h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <form action="TipoServlet">
            <input type="hidden" name="idtipo" value="<%=item.getIdtipo()%>">
    ...
    <div class="modal fade" id="modal-delete<%=count%>" tabindex="-1" aria-labelledby="modal-titleD<%=count%>" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="modal-titleD<%=count%>">¿Eliminar al Tipo Nro. <%=item.getIdtipo() %>?</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <form action="TipoServlet">
            <input type="hidden" name="idtipo" value="<%=item.getIdtipo()%>">
